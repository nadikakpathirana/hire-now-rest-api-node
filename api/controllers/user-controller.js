    ...
    .then((user) => {
      if (user.length >= 1) {
        ...
        if (user[0].email == req.body.email) {
          err.error.errors.email = { kind: "is already taken", path: "email" };
        }
        if (user[0].username == req.body.username) {
          err.error.errors.username = {
            kind: "is already taken",
            path: "username",
          };
        }
            ...
            user
              ...
              .then((result) => {
                ...
                let mailOptions = {
                  ...
                  html: `<p>Click <a href=http://localhost:3000/emailverify/${a}/${b}>here</a> to verify your email address</p>`,
                };
                console.log("aaaaaa");
    ...
    .then(async (doc) => {
      if (doc) {
        if (doc.userType == "seller") {
          ...
          doc.rating = !isNaN(Number(totValues / totRatings).toFixed(1))
